...
import connect, {Db} from './modules/db/index';
...
export default async() => {
  ...
  // db
  await app.register(connect, {
    URL: app.config.DB_STRING,
    DB_USER: app.config.DB_USER,
    DB_PASS: app.config.DB_PASS,
    DB_PASS_PATH: app.config.DB_PASS_PATH,
    DB_ROOT_CERT_PATH: app.config.DB_ROOT_CERT_PATH
  });

  // auth
  const authenticate = {realm: 'reflect'};
  const validate = async(username:string, password:string) => {
    if(username !== app.config.USER || password !== app.config.PASSWORD) {
