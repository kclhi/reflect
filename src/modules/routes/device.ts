...
export default async(server:FastifyInstance) => {
  ...
  server.route({url:'/register', method:['GET'], handler:(req, rep)=>{
    ...
    const withingsRedirectUrl = server.config.WITHINGS_AUTHORISATION_URL+'?response_type=code&redirect_uri='+callback+'&client_id='+server.config.WITHINGS_CLIENT_ID+'&scope=user.info,user.metrics,user.activity&state='+state.toString('hex');
    ...
    rep.view('register.pug', {withingsRedirectUrl:withingsRedirectUrl, garminRedirectUrl:'/connect/garmin', patientId:patientId});
  }});
