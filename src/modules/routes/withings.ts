  ...
  server.route<{Querystring:CallbackType}>({
    ...
    handler: async(req, rep) => {
      ...
      if(req.cookies&&Object.keys(req.cookies).includes(server.config.PATIENT_ID_COOKIE)&&req.unsignCookie(req.cookies[server.config.PATIENT_ID_COOKIE]).valid) {
        let access:TokenResponseBody|undefined = await Withings.getAccessToken(server.config.WITHINGS_TOKEN_URL, server.config.WITHINGS_CLIENT_ID, server.config.WITHINGS_CONSUMER_SECRET, server.config.WITHINGS_CALLBACK_BASE_URL, req.query.code);
        if(access) {
          ...
          try {
            let subscribed:boolean = await Withings.subscribeToNotifications(server.config.WITHINGS_SUBSCRIPTION_URL, access.access_token, access.userid, server.config.API_URL);
