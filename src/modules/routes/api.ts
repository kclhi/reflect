...
export default async(server:FastifyInstance) => {
  ...
  server.route<{Body:PatientIdType, Reply:FHIRResourceType}>({url:'/fhir', method:['POST'], schema:{body:PatientId, response:{200:FHIRResource}}, handler:async(_req, rep)=>{
    let getFHIR:Response<string>|undefined;
    try {
      getFHIR = await got.get('https://'+server.config.INTERNAL_API_URL+'/fhir/Observation?subject=foo');
