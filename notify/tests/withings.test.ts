...
describe('withings', () => {
  it('should be able to translate data', async() => {
    const translated:WithingsData = Withings.translate(
      JSON.parse(await fs.readFile(path.resolve(__dirname, './fixtures/withings-data-response.json'), 'utf8')).body
    );
    ...
    expect(translated.measuregrps[0].attrib).to.equal(
