import { ContextPayload, EventPayload, Config, Notification, PatientId, Token, NokiaData, BloodPressureReading } from './types';
...
export default async(event:EventPayload, context:ContextPayload):Promise<ContextPayload> => {
  ...
  // ~mdc send positive response to nokia regardless (also acks initial HEAD request from nokia when setting up notifications)
  ...
  try {
    ...
    getPatientId = await got.post<PatientId>(process.env.internal_service_url+'/id', {json:{"nokiaId":userId}, username:process.env.user, password:process.env.password, responseType:'json'});
  ...
  try {
    let userId:string = (event.body as Notification).userid;
    getToken = await got.post<Token>(process.env.internal_service_url+'/token', {json:{"nokiaId":userId}, username:process.env.user, password:process.env.password, responseType:'json'});
