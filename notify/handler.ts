...
export default async(event:EventPayload, context:ContextPayload):Promise<ContextPayload> => {
  ...
  try {
    getPatientId = await got.post<PatientId>(process.env.internal_service_url+'/id/withings', {json:{"vendorId":userId}, username:process.env.user, password:process.env.password, responseType:'json'});
  ...
  try {
    getToken = await got.post<Token>(process.env.internal_service_url+'/token', {json:{"vendorId":userId}, username:process.env.user, password:process.env.password, responseType:'json'});
  ...
  // get data
  let params:any = {action:'getmeas', userid:notification.userid, [config.WITHINGS_API_DATA.START.getmeas]:notification.startdate, [config.WITHINGS_API_DATA.END.getmeas]:notification.enddate};
