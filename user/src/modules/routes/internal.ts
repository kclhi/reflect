...
export default async(server:FastifyInstance) => {
  ...
  server.route<{Body:VendorIdType; Reply:PatientIdType}>({
    url: '/id/withings',
    method: ['POST'],
    schema: {body: VendorId, response: {200: PatientId}},
  ...
  server.route<{Body:VendorIdType; Reply:TokenType}>({
    url: '/token',
    method: ['POST'],
    schema: {body: VendorId, response: {200: Token}},
    handler: async(req, rep) => {
      const {body: withings} = req;
      ...
      const access:TokenResponseBody | undefined = await WithingsUtils.refreshAccessToken(
      ...
      try {
        await Withings.updateOne(
          {withingsId: user.withingsId},
          {token: access.access_token, refresh: access.refresh_token},
          {upsert: true}
        );
