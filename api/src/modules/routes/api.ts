...
export default async(server:FastifyInstance) => {
  ...
  server.route<{Body:PatientIdType; Reply:FHIRResourceType}>({
    url: '/fhir',
    method: ['POST'],
    schema: {
      description: 'Collect all the data available on a patient. Formatted as FHIR.',
      body: PatientId,
      response: {200: FHIRResource},
      security: [{bearerAuth: []}]
    },
